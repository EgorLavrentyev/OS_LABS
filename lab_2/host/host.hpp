...
class Host {
  public:
    ...
    void start_game() {
        ...
        std::cout << "Количество козлят: " << active_clients << " шт." << std::endl << std::endl;

        while (active_clients > 0) {
            size_t wolf_roll = (size_t)-1;
            pthread_t read_thread;
            std::cout << "Число для волка: ";
            ...
            std::cout << std::endl << "Волк выбросил число: " << wolf_roll << std::endl;
            ...
            for (auto &cl_info : cl_info_vec) {
                ...
                std::cout << "Козлёнок " << cl_info.client_id << " выбросил число: " << msg.roll << std::endl;
                ...
                auto bool_to_str = [](bool val) -> std::string { return val ? "Да" : "Нет"; };
                std::cout << "Козлёнок " << cl_info.client_id << " жив: " << bool_to_str(msg.alive) << std::endl;
                ...
                std::cout << "Козлёнок " << cl_info.client_id << " мёртв подряд: " << cl_info.death_row << std::endl;

                if (cl_info.death_row > 1) {
                    std::cout << "Козлёнок " << cl_info.client_id << " мёртв" << std::endl;
                    msg.active = false;
            ...
            std::cout << "Конец раунда" << std::endl << std::endl;
        }
